# Feedback Log

Consolidated feedback from replies to daily briefing emails. Updated automatically during the daily routine. Manually harvested by Eve to refine project plans and prompt instructions.

---

## Suppressions
_Items to omit from future briefings_

<!-- e.g. **2026-05-13** | Skip the Markus App Store deadline until it's actually on the calendar. -->

---

## Deprioritizations
_Items lower priority than the plans currently suggest_

<!-- e.g. **2026-05-13** | MCP Gateway revival is fully parked — treat like teardown queue. -->

---

## Context Requests
_Requests for more depth — marked OPEN until addressed_

**2026-05-13** | CLOSED: Is TABF compatible with the Xcode Claude agent? Does it run from a repo? (Answered in May 13 briefing: TABF runs via Claude Code from the project root; Xcode project is a standard git repo; compatible. Confirmed by Eve in reply.)

---

## General Notes
_Tone, framing, and structural preferences_

**2026-05-13** | May 18 second appointment (12:50pm, 904 7th Ave 2nd Floor) is a mammogram and bloodwork — do not wear deodorant, lotion, or powder.

**2026-05-13** | New role activation window = when ShinShin is at work. Schedule new role work for those windows, not when she's home with guests.

**2026-05-13** | Open question framing for Pullman: Eve is going in with an open mind and focus on objectives — don't pre-assume which specific tactics or follow-up tasks will be needed.

**2026-05-13** | Commission: Eve asked Sam (LGBTQ Commission) about creating a commissioner workgroup for between-meeting work on SOGI data. Update when answer is known.

**2026-05-17** | Flying Stick: MCP Gateway rebuild (Apple Music + GitHub) is important and should be kept visible in the queue behind TABF.

**2026-05-17** | Commission/Pullman framing: Commissioners acknowledged need for SOGI data but raised federal exposure concerns. Key framing going forward: collect only what is fit-for-purpose; structure collection so it does not create data that would require sharing with federal authorities.

**2026-05-17** | Commission workgroup confirmed: Staff agreed to follow up and create a commissioner workgroup after the weekend. Sam call Monday is the formal follow-up step.

**2026-05-17** | Flying Stick/TABF: Eve has started working through the TABF materials — reading, editing, cleaning up. Still in progress as of May 16.

**2026-05-19** | Commission — Eve emailed Commissioner Archer to request a meeting or call about SOGI data interest. Outreach sent; awaiting reply.

---

## Processing Log
_Internal record — tracks which briefing threads have been checked for replies_

**2026-05-13** | Processed reply to "[Daily Focus] Wednesday, May 13". 5 items added (1 Context Request, 4 General Notes).

**2026-05-17** | Processed reply to "[Daily Focus] Thursday, May 14". 1 item added (1 General Note).

**2026-05-17** | Processed reply to "[Daily Focus] Friday, May 15". 3 items added (3 General Notes).

**2026-05-19** | Processed reply to "[Daily Focus] Monday, May 18". 1 item added (1 General Note).
